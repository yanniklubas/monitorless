...
while [ $exit -eq 0 ]; do
	set +e
	if cqlsh -f /setup_tables.txt "$first_server"; then
		exit=1
	else
...
/ycsb/bin/ycsb.sh load cassandra-cql -p hosts="$1" -P /ycsb/workloads/"$WORKLOAD" -p recordcount="$2" -s -threads "$THREADS"
